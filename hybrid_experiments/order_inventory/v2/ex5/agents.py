...
def tool_update_order_status(order_id, status):
    with httpx.Client(timeout=5) as c:
        r = c.post(f"{ORDER_SERVICE_URL}/update_status", json={"order_id": order_id, "status": status})
        r.raise_for_status()
        return r.json()
...
def order_node(state: OrderState):
    key = f"order_agent:{state['order_id']}"
    state = state_store.load_state(key) or state
    if "history" not in state: state["history"] = []
    if "trace" not in state: state["trace"] = []

    prompt = f"""You are Order Agent, Based on the status in current state choose the correct action from tools: 
             if status is None: choose action as create_order 
    ...
    if action == "create_order":
        ...
        state["trace"].append({"step": "create_order", "out": out})
...
def inventory_node(state: OrderState):
    key = f"inventory_agent:{state['order_id']}"
    state = state_store.load_state(key) or state
    if "history" not in state: state["history"] = []
    if "trace" not in state: state["trace"] = []

    prompt = f"""You are Inventory Agent, Based on the status in current state choose the correct action from tools: 
    ...
    if action == "reserve_inventory":
        ...
        state["trace"].append({"step": "reserve_inventory", "out": out})
        state["status"] = out.get("status", "INIT")
        ...
        state["trace"].append({"step": "update_order_status", "out": status_update})
...
def build_graph():
    ...
    def order_next(state: OrderState):
        if state["status"] in ["INIT", "reserved"]:
            return "inventory_agent"
        return END

    workflow.add_conditional_edges("order_agent", path=order_next)
